...
1. Collect the lazy query before pivoting, then re-wrap in lazy if needed:

```python
df.lazy().collect().pivot(...).lazy()
```

2. Pre-declare the expected column values with `on_columns=`:
