...
For better presentation, this book use polars' [config options](https://docs.pola.rs/py-polars/html/reference/config.html) to change the formatting of printed tables. Specifically:

- Limit the number of columns displayed in DataFrames to 10
- Cap the number of rows shown in DataFrames to 10
...
If you want to take a peek at different parts of your DataFrame, here's a neat trick: use the `sample()` method. This method
randomly grabs `n` number of rows from the DataFrame and returns them for inspection.
...
like `.max()`, `.mean()`, as well as specialized ones such as `.quantile()` and `.unique_counts()`.
